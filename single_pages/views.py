...
def best(request):
    ...
    return render(request, 'single_pages/best.html', {'best_foods': best_foods, 'categories':  categories})

# 마이페이지
def mypage(request):
    food_list = [] # 찜한 상품 목록
    order_list = Order.objects.filter(user=request.user).order_by('-created_at') # 로그인한 사용자의 주문 목록(최신순으로)

    for food in Food.objects.all():
        if request.user in food.like_users.all():
            food_list.append(food)

    return render(request, 'single_pages/mypage.html', {'categories':  categories, 'food_list': food_list, 'order_list': order_list})

# 회사소개
def company(request):
    best_foods = Food.objects.order_by('-sales')[:5] # 판매량이 높은 상품 5개
    labels1 = [] # 카테고리명
    data1 = [] # 각 카테고리의 상품 개수
    labels2 = [] # 상품명
    data2 = [] # 각 상품의 판매량
...
def cart(request):
    ...
    final_price = 0 # 총 상품금액(품절된 상품 제외)
...
def check_order(request):
    if request.user.is_authenticated: # 로그인을 한 경우
        ...
        # 주문 가능한 수량인지 확인
        for cart in cart_list:
            if cart.food.inventory > 0 and cart.quantity > cart.food.inventory: # 주문이 불가능하다면 메세지를 띄운다
                messages.info(request, str(cart.food.name) + '의 수량이 재고수량(' + str(cart.food.inventory) + ')를 초과합니다.')
                return redirect('/cart/')
        else: # 주문이 가능하면 주문하기로 넘어간다
...
class OrderCreate(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        if self.request.user.is_authenticated:
            ...
            cart_list = Cart.objects.filter(user=self.request.user, food__inventory__gt=0) # 재고량이 1개 이상인 상품이 담긴 장바구니 목록
